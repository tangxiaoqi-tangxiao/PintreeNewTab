...
## 主题色
项目品牌主题色为绿色 `#0BA665`（`rgb(11, 166, 101)`）。

- **使用位置**：加载 spinner（`border-top-color`）、文件夹图标（`fill`）、侧边栏选中态（`text-main-500`）、`.text-main-500` 类、`.btn-brand` 绿色按钮等。
- **`.btn-brand` 按钮**（styles.css 定义）：背景 `rgba(11, 166, 101)`，白字，hover `#0a9457`，active `#09804b`。用于需要强调品牌色的操作按钮（如新功能提醒的"知道了"）。
- **深色模式不改变品牌绿**，主题色在明暗模式下保持一致。
- 需要强调色时优先使用品牌绿，避免混入无关色相。
